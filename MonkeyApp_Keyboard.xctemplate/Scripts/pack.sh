...
function pack(){
	...
	# local KEYBOARDEXTENSION=`ls ${BUILT_PRODUCTS_DIR} | grep .appex`
	# if [ "${KEYBOARDEXTENSION}" != "" ];then
	# 	if [ -d "${BUILD_APP_PATH}/PlugIns" ];then
	# 		rm -rf ${BUILD_APP_PATH}/PlugIns
	# 		mkdir ${BUILD_APP_PATH}/PlugIns
	# 	else
	# 		mkdir ${BUILD_APP_PATH}/PlugIns
	# 	fi
	# 	cp -rf "${BUILT_PRODUCTS_DIR}/${KEYBOARDEXTENSION}" "${BUILD_APP_PATH}/PlugIns/${KEYBOARDEXTENSION}"
	# fi
