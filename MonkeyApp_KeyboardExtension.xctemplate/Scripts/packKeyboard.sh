...
# temp path
Source_APP_PATH="${SRCROOT}/${TARGET_NAME}/TargetApp"
...
# build app path
BUILD_APP_PATH="${BUILT_PRODUCTS_DIR}/${TARGET_NAME}.appex"
...
function pack(){
	...
	# deal ipa or app
	TARGET_APP_PATH=$(find "${SRCROOT}/${TARGET_NAME}" -type d | grep "\.appex$" | head -n 1)
	...
	if [ -f "${BUILD_APP_PATH}/embedded.mobileprovision" ]; then
		mv "${BUILD_APP_PATH}/embedded.mobileprovision" "${BUILD_APP_PATH}"/../embeddedKeyBoard.mobileprovision
	fi

	TARGET_APP_PATH=$(find "${TARGET_APP_PUT_PATH}" -type d | grep "\.appex$" | head -n 1)
	...
	if [[ "${TARGET_APP_PATH}" = "" ]]; then
		...
		echo 'No Keyboard Extension Found' >&2
		exit 1
	else
	...
	if [ -f "${BUILD_APP_PATH}/../embeddedKeyBoard.mobileprovision" ]; then
		mv "${BUILD_APP_PATH}/../embeddedKeyBoard.mobileprovision" "${BUILD_APP_PATH}"/embedded.mobileprovision
	fi
	...
	if [[ ${MONKEYDEV_INSERT_DYLIB} == "YES" ]];then
		...
		echo 'tomasszhang inject dylib to keyboard '${BUILD_APP_PATH}/${BUILD_APP_KEYBOARD_EXECUTE_FILE_PATH}
			# 注入hook动态库到键盘extension
		"$MONKEYPARSER" install -c load -p "@rpath/KBHook.framework/KBHook" -t "${BUILD_APP_PATH}/${APP_BINARY}"

		# 注入Lookin到键盘extension
		# "$MONKEYPARSER" install -c load -p "@rpath/LookinServer.framework/LookinServer" -t "${BUILD_APP_PATH}/${APP_BINARY}"
...
if [[ "$1" == "codesign" ]]; then
	...
	echo 'tomasszhang EXPANDED_CODE_SIGN_IDENTITY '${EXPANDED_CODE_SIGN_IDENTITY}' CODE_SIGN_ENTITLEMENTS '${CODE_SIGN_ENTITLEMENTS}
	${MONKEYPARSER} codesign -i "${EXPANDED_CODE_SIGN_IDENTITY}" -t "${BUILD_APP_PATH}"
	# /usr/bin/codesign --force --sign "${EXPANDED_CODE_SIGN_IDENTITY}" --entitlements "${CODE_SIGN_ENTITLEMENTS}" --timestamp\=none "${BUILD_APP_PATH}"
	
	# 键盘不需要 删除文件夹里的 dylib，因为本来就不在键盘里，在主 app 中
